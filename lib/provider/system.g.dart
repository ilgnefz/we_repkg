// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'system.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$currentStateHash() => r'dbb8ca2577fb8aa29f650711019cd4349c993704';

/// See also [CurrentState].
@ProviderFor(CurrentState)
final currentStateProvider =
    AutoDisposeNotifierProvider<CurrentState, RunState>.internal(
      CurrentState.new,
      name: r'currentStateProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$currentStateHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

typedef _$CurrentState = AutoDisposeNotifier<RunState>;
String _$wallpaperPathHash() => r'3a2731c53aa4a9c1a1b331685f333c10139919ec';

/// See also [WallpaperPath].
@ProviderFor(WallpaperPath)
final wallpaperPathProvider =
    AutoDisposeNotifierProvider<WallpaperPath, String?>.internal(
      WallpaperPath.new,
      name: r'wallpaperPathProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$wallpaperPathHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

typedef _$WallpaperPath = AutoDisposeNotifier<String?>;
String _$toolPathHash() => r'9fe6c0204e0fa16031d71e4c345333ccd03b0d45';

/// See also [ToolPath].
@ProviderFor(ToolPath)
final toolPathProvider =
    AutoDisposeNotifierProvider<ToolPath, String?>.internal(
      ToolPath.new,
      name: r'toolPathProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$toolPathHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

typedef _$ToolPath = AutoDisposeNotifier<String?>;
String _$projectPathHash() => r'085f435dd97ea2362873b41da52f40ac23779025';

/// See also [ProjectPath].
@ProviderFor(ProjectPath)
final projectPathProvider =
    AutoDisposeNotifierProvider<ProjectPath, String?>.internal(
      ProjectPath.new,
      name: r'projectPathProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$projectPathHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

typedef _$ProjectPath = AutoDisposeNotifier<String?>;
String _$exportPathHash() => r'6903ac90bbd8b6d8b7488e1a86f585e3fb535d84';

/// See also [ExportPath].
@ProviderFor(ExportPath)
final exportPathProvider =
    AutoDisposeNotifierProvider<ExportPath, String?>.internal(
      ExportPath.new,
      name: r'exportPathProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$exportPathHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

typedef _$ExportPath = AutoDisposeNotifier<String?>;
String _$toolVersionHash() => r'a17f17793c844f453aa35db57e58625a9b450fe4';

/// See also [ToolVersion].
@ProviderFor(ToolVersion)
final toolVersionProvider =
    AutoDisposeNotifierProvider<ToolVersion, String?>.internal(
      ToolVersion.new,
      name: r'toolVersionProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$toolVersionHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

typedef _$ToolVersion = AutoDisposeNotifier<String?>;
String _$earliestTimeHash() => r'9708c6622db404fffae34898ad3716ecdd896009';

/// See also [EarliestTime].
@ProviderFor(EarliestTime)
final earliestTimeProvider =
    AutoDisposeNotifierProvider<EarliestTime, String?>.internal(
      EarliestTime.new,
      name: r'earliestTimeProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$earliestTimeHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

typedef _$EarliestTime = AutoDisposeNotifier<String?>;
String _$searchContentHash() => r'2ad72268d8e98b714d994b87f12d0559a6f2a853';

/// See also [SearchContent].
@ProviderFor(SearchContent)
final searchContentProvider =
    AutoDisposeNotifierProvider<SearchContent, String>.internal(
      SearchContent.new,
      name: r'searchContentProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$searchContentHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

typedef _$SearchContent = AutoDisposeNotifier<String>;
String _$currentThemeHash() => r'1abdaab8aa66cc869001aa98a9d319789a6000eb';

/// See also [CurrentTheme].
@ProviderFor(CurrentTheme)
final currentThemeProvider =
    AutoDisposeNotifierProvider<CurrentTheme, ThemeType>.internal(
      CurrentTheme.new,
      name: r'currentThemeProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$currentThemeHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

typedef _$CurrentTheme = AutoDisposeNotifier<ThemeType>;
String _$currentLanguageHash() => r'52d41ffe10bd800f6d9e78072c62c7e50c4984b8';

/// See also [CurrentLanguage].
@ProviderFor(CurrentLanguage)
final currentLanguageProvider =
    AutoDisposeNotifierProvider<CurrentLanguage, LanguageType?>.internal(
      CurrentLanguage.new,
      name: r'currentLanguageProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$currentLanguageHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

typedef _$CurrentLanguage = AutoDisposeNotifier<LanguageType?>;
String _$loadingTextHash() => r'8f1b694310b720018a867ff9f337c360d7117242';

/// See also [LoadingText].
@ProviderFor(LoadingText)
final loadingTextProvider =
    AutoDisposeNotifierProvider<LoadingText, String>.internal(
      LoadingText.new,
      name: r'loadingTextProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$loadingTextHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

typedef _$LoadingText = AutoDisposeNotifier<String>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member, deprecated_member_use_from_same_package
